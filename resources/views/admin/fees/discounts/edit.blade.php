...
<head>
    ...
    <title>Edit Discount - E-Fees Portal</title>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main class="flex-1 p-4 md:p-8 overflow-y-auto bg-slate-50 custom-scrollbar">
            <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit Discount Rule</h1>
                    <p class="text-gray-500 mt-1">Update details for {{ $discount->discount_name }}</p>
                </div>
                <div class="flex items-center gap-3">
                    <form method="POST" action="{{ route('admin.fees.destroy-discount', $discount->id) }}" onsubmit="return confirm('Are you sure you want to delete this discount? This action cannot be undone.');">
                        @csrf
                        @method('DELETE')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <form method="POST" action="{{ route('admin.fees.update-discount', $discount->id) }}" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <input type="text" name="discount_name" id="discount_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" value="{{ old('discount_name', $discount->discount_name) }}" required>
                        </div>
                        ...
                        <div x-data="{ isPercentage: '{{ old('type', $discount->type) }}' === 'percentage' }" @type-change.window="isPercentage = ($event.detail === 'percentage')">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="value" id="value" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" :class="!isPercentage ? 'pl-7' : ''" value="{{ old('value', $discount->value) }}" step="0.01" required>
                        ...
                        <div class="col-span-1 md:col-span-2" x-data="{ scope: '{{ old('apply_scope', data_get($discount->eligibility_rules, 'apply_scope', 'total')) }}' }">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    @foreach($additionalCharges as $charge)
                                        <label class="inline-flex items-center">
                                            <input type="checkbox" name="target_charge_ids[]" value="{{ $charge->id }}" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500" {{ in_array($charge->id, old('target_charge_ids', data_get($discount->eligibility_rules, 'target_charge_ids', []))) ? 'checked' : '' }}>
                        ...
                        <div class="col-span-1 md:col-span-2" x-data="{ showGrades: {{ old('is_automatic', $discount->is_automatic) ? 'true' : 'false' }} }" @toggle-grades.window="showGrades = $event.detail" x-show="showGrades" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Applicable Grades</label>
                            <div class="grid grid-cols-3 md:grid-cols-6 gap-2">
                                @foreach($gradeLevels as $grade)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="applicable_grades[]" value="{{ $grade }}" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500" {{ in_array($grade, old('applicable_grades', $discount->applicable_grades ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('description', $discount->description) }}</textarea>
                        </div>

                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" {{ old('is_active', $discount->is_active) ? 'checked' : '' }}>
