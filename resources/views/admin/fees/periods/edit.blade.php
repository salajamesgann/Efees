...
<head>
    ...
    <title>Edit Fee Period - E-Fees Portal</title>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto bg-slate-50 p-6 lg:p-8">
            <header class="mb-8 flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit Fee Period</h1>
                    <p class="text-gray-500 mt-1">Update School Year or Semester.</p>
                </div>
            ...
            <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <form action="{{ route('admin.fees.periods.update', $feePeriod) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="type" id="type" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                <option value="school_year" {{ old('type', $feePeriod->type) == 'school_year' ? 'selected' : '' }}>School Year</option>
                                <option value="semester" {{ old('type', $feePeriod->type) == 'semester' ? 'selected' : '' }}>Semester</option>
                            </select>
                        </div>

                        <div>
                            <label for="label" class="block text-sm font-medium text-gray-700 mb-1">Label</label>
                            <input type="text" name="label" id="label" value="{{ old('label', $feePeriod->label) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" placeholder="e.g. 2024-2025 or 1st Semester">
                        </div>

                        <div>
                            <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-1">Sort Order</label>
                            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $feePeriod->sort_order) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                        </div>

                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" {{ old('is_active', $feePeriod->is_active) ? 'checked' : '' }}>
