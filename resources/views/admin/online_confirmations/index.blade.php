...
<head>
  ...
  <title>Online Payment Confirmations - Efees Admin</title>
      ...
      <main class="flex-1 p-6 lg:p-8 md:h-screen overflow-y-auto bg-gray-50 custom-scrollbar">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Online Payment Confirmations</h1>
                    <p class="text-sm text-slate-500 mt-1">Review and confirm pending online payments via PayMongo</p>
                </div>
            </div>

            <!-- Filters & Search -->
            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                <form action="{{ route('admin.online_confirmations.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($confirmations as $payment)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-slate-500">{{ $payment->created_at->format('M d, Y h:i A') }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <form action="{{ route('admin.online_confirmations.confirm', $payment->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to confirm this online payment?');">
                                                @csrf
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="font-medium">No online payments awaiting confirmation</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          <form :action="`{{ url('admin/online-confirmations') }}/${paymentId}/reject`" method="POST">
              @csrf
              <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                  ...
                  <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                    <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Reject Online Payment</h3>
                    <div class="mt-2">
                      <p class="text-sm text-gray-500">Are you sure you want to reject this online payment? This will notify the parent and the payment will not be applied.</p>
                      <div class="mt-4">
