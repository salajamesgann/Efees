...
<body class="flex flex-col md:flex-row min-h-screen bg-gray-50 text-gray-900" x-data="{ sidebarOpen: false }">
  ...
  <main class="flex-1 p-8 md:h-screen overflow-y-auto custom-scrollbar">
    ...
    <form method="POST" action="{{ route('super_admin.users.store') }}" class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 card-hover" novalidate x-data="{ submitting: false }" @submit="submitting = true">
      @csrf
